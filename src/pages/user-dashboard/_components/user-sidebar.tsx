            ...
            <div className="w-0 hidden lg:block md:w-[320px] barlow fixed bg-[#1c2434] overflow-y-scroll  h-screen pt-4 px-4">
                <div className="flex flex-col space-y-6 mt-8">
                    ...
                    {data?.data.map((link: any) => (
                        <Link to={`/user-dashboard/${link.id}`} key={link.id}>
                            <UserSidebarItem icon={CirclePlay} label={link.title} path={`user-dashboard/${link.id}`} />
                        </Link>
                    ))}
                    ...
                    <Link to="/user-dashboard/settings">
                        <UserSidebarItem icon={Settings} label="Settings" path="/user-dashboard/settings" />
                    </Link>
                </div>
            </div>
        </div>
